...
// ★ 被动遗物测试区
// ============================================================
// 布局：每个遗物生成多个，用来看叠加（×2 ×3 ...）
// 参数：item_type="relic_passive"  item_id=遗物ID  x, y  rarity
//
// ★ 品质策略：**均匀轮换 7 档品质**（不是 data_rarity_roll 的加权随机）。
//   理由：测试目的是看"品质色背景/边框"是否正确跟着变，
//   加权随机大部分出 common，看不全。轮换能一次看遍所有品质色。
//
// 坐标规划（避开上面已有的主动遗物 700~900, 800）：
//   太阳能板      1200, 800   ×3   ← 可叠加
//   风力发电机    1200, 950   ×2   ← ★ 不可叠加（验证"重复拾取只提示"）
//   过载电容      1400, 800   ×3   ← 可叠加
//   动能回收器    1400, 950   ×3   ← 可叠加
//   超频颗粒      1600, 800   ×3   ← 可叠加

var _all_rarities = ["common", "rare", "elite", "epic", "legendary", "mythic", "divine"];

var _passive_relics = [
    { id: "relic_solar_panel",        x: 1200, y: 800, count: 3 },
    { id: "relic_wind_turbine",       x: 1200, y: 950, count: 2 },
    { id: "relic_overload_capacitor", x: 1400, y: 800, count: 3 },
    { id: "relic_kinetic_recovery",   x: 1400, y: 950, count: 3 },
    { id: "relic_overclock_particle", x: 1600, y: 800, count: 3 },
];

var _rarity_cursor = 0;     // 全局轮换游标 → 保证每个品质都能出现

for (var _i = 0; _i < array_length(_passive_relics); _i++) {
    var _entry = _passive_relics[_i];
    for (var _n = 0; _n < _entry.count; _n++) {
        var _rarity = _all_rarities[_rarity_cursor mod array_length(_all_rarities)];
        _rarity_cursor += 1;

        // 每个之间错开一点，避免完全重叠看不出数量
        scr_item_create_on_ground("relic_passive", _entry.id,
                                  _entry.x + (_n mod 3) * 50,
                                  _entry.y + (_n div 3) * 50,
                                  _rarity);
    }
}
