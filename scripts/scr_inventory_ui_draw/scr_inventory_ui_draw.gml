...
function scr_inventory_ui_draw(ui) {
    ...
    // ===== 5b. ★ 被动遗物栏（背包右侧）=====
    scr_draw_passive_relic_panel(ui, _player);
    
    // ===== 6. 悬浮详情 =====
    scr_draw_hover_tooltip(ui, _player);
    
    // ★ 被动遗物悬浮说明（纯数据，不能走 scr_draw_item_tooltip）
    scr_draw_passive_relic_tooltip(ui, _player);
...
scr_draw_confirm_dialog(ui);
    ...
    // ===== 8b. ★ 被动遗物拆卸确认框（比普通确认框更上层）=====
    scr_draw_passive_dismantle_dialog(ui, _player);
    // ===== 9. 重置绘图状态 =====
    draw_set_color(c_white);
    draw_set_alpha(1);
