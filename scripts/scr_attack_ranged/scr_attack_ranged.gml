...
function scr_attack_ranged(_owner, _weapon = noone) {
    ...
    // ★ 过载电容：攻击消耗电量换伤害（仅玩家；放在扣弹之前，不允许就整个中止）
    var _overload_mult = 1.0;
    var _is_enemy_ranged = object_is_ancestor(_owner.object_index, obj_enemy_base);
    if (!_is_enemy_ranged) {
    ...
    if (instance_exists(_bullet)) {
        ...
        _bullet.life = _data.bullet_life;
                _bullet.damage = _weapon.final_stats.base_damage * scr_relic_get_damage_mult(_owner) * _overload_mult;
