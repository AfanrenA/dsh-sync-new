// scr_attack_hack.gml
function scr_attack_hack(_owner, _weapon = noone) {
    ...
    if (_is_enemy) {
        ...
        show_debug_message("[HACK ATTACK] 敌人攻击 | 冷却倍率: " + string(_cooldown_multiplier));
    }
    
    // ★ 过载电容：攻击消耗电量换伤害（仅玩家，真的能攻击时才结算）
    ...
    var _hitbox_life = (_data.hitbox_life != undefined) ? _data.hitbox_life : 12;
    var _hitbox_behavior = (_data.hitbox_behavior != undefined) ? _data.hitbox_behavior : "destroy_projectiles";
    ...
    show_debug_message("[HACK ATTACK] 启动 | 武器: " + _weapon.weapon_id);
}