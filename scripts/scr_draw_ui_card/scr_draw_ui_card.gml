...
function scr_draw_ui_card(x, y, w, h, name, border_color, text_color, cooldown_progress = 1, flash_timer = 0, can_flash = true) {
    ...
    // ★ 按"框宽"自适应截断（原来是死写 6 个字，导致长名字溢出、短名字浪费空间）
    //   逻辑：用实际字体宽度判断，超出可用宽度才截断并加 ".."
    var _text_max_w = w - 8;                 // 左右各留 4px 呼吸
    if (string_width(_display_name) > _text_max_w) {
        var _len = string_length(_display_name);
        var _cut = _len;
        // 逐字回退，找到能放下 "... " 的最大长度
        while (_cut > 0 && string_width(string_copy(_display_name, 1, _cut) + "..") > _text_max_w) {
            _cut -= 1;
        }
        if (_cut <= 0) {
            _display_name = "";
        } else {
            _display_name = string_copy(_display_name, 1, _cut) + "..";
        }
    }
