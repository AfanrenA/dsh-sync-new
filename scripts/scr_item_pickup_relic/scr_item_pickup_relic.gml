...
/// @description 拾取遗物：主动→装槽；被动→转成池数据
function scr_item_pickup_relic(player, item) {
    if (!instance_exists(player)) return;
    if (!instance_exists(item)) return;
    if (!item.is_on_ground) return;

    // ===== ★ 被动遗物分流：转成池数据并销毁物品，不走槽位逻辑 =====
    if (variable_instance_exists(item, "is_passive") && item.is_passive) {
        scr_relic_passive_pickup_from_item(player, item);
        return;
    }
